  ...
  for (let i = 0; i < fileSize; i += maxMemUsage) {
    ...
    for (let j = 0; j <= currentChunk.length - 1; j++) {
      if (currentChunk.charCodeAt(j) === '10') {
        resArr[resArr.length - 1].end = i + j - prevSymbolIsNewStr
        resArr.push({
          start: i + j < fileSize - 1 ? j + i + 1 : j,
          end: undefined
        })
...
async function sortArr (arr, inputFile) {
  while (arr.length > 1) {
    ...
    for (let i = 0; i < arr.length; i += 2) {
      if (i + 1 < arr.length) { tempArr.push(await merge(arr[i], arr[i + 1], inputFile)) } else tempArr.push(arr[i])
    }
...
async function merge (left, right, fileName) {
  ...
  while (i < left.length || j < right.length) {
    ...
    if (await stringsCompare(left[i], right[j], fileName)) { resArr.push(left[i++]) } else { resArr.push(right[j++]) }
  }
}

async function stringsCompare (left, right, file) {
  const res = left.end - left.start <= right.end - right.start
  ...
  for (let i = 0; i <= endCycle; i++) {
    ...
    if (leftSymbol !== rightSymbol) { return leftSymbol.charCodeAt(0) < rightSymbol.charCodeAt(0) }
  }
...
await bigSort(process.argv[2], process.argv[3])
